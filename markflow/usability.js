...
window.installMarkflowUsability=function(api){
  ...
  document.documentElement.dataset.markflowUi='3.9.0';
  let anchor=null;
  ...
  button(format,'굵게','<b>B</b>',()=>api.exec('bold'));button(format,'기울임','<i>I</i>',()=>api.exec('italic'));button(format,'형광펜','<span class="mf-highlight-icon">가</span>',b=>api.highlight(b.getBoundingClientRect()));
  ...
  const version=document.querySelector('.status');if(version){const nodes=[...version.querySelectorAll('*')];const old=nodes.find(el=>el.children.length===0&&el.textContent.trim()==='v3.8');if(old)old.textContent='v3.9';}
};
