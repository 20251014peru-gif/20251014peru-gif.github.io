...
## 투자 기록보관실 읽기 전용 연동 — 0.4

server/investment-feed.mjs는 별도 프로그램(github.com/20251014peru-gif/invest, records.html "투자 기록보관실")이 **같은 Firebase 프로젝트**(my-system-25497)에 쓰는 최상위 컬렉션 `records`(종목 아이디어·복기 기록)와 `records_todos`(날짜별 할 일)를 읽어 캘린더 이벤트 모양으로 바꿉니다.

- 한쪽 방향만입니다: 여기서 읽기만 하고, 그 컬렉션에 쓰지 않습니다. 달님 캘린더 자체의 `dalnimSpaces/{space}/events`에도 저장하지 않습니다 — 매 조회마다 두 컬렉션을 다시 읽어 그 자리에서 합쳐 보여줄 뿐입니다.
- `GET /investment-feed`는 공간 role이 owner일 때만 허용합니다. 가족 공유(family) 일정과 달리, 초대된 editor/viewer 계정에는 투자 기록이 보이지 않습니다.
- 최근 400일, 각 컬렉션 최대 1500/500건으로 조회 범위를 제한해 Firestore 읽기 비용을 억제합니다. 앱을 새로 열거나 클라우드에 로그인할 때 한 번만 불러오고, 15초 주기 폴링에는 포함하지 않습니다.
- 반환되는 이벤트는 `readOnly:true`이며 화면에서 클릭하면 수정 폼이 아니라 보기 전용 미리보기가 뜹니다. 드래그·크기 조절도 기존 `editable:...&&!e.readOnly` 조건으로 막혀 있습니다.
- invest 저장소의 `records`/`records_todos` 스키마가 바뀌면(예: 필드명 변경) 이 매핑 함수도 같이 고쳐야 합니다 — 자동으로 맞춰지지 않습니다.
