...
export const calendarApi=onRequest({region:'asia-northeast3',maxInstances:3,timeoutSeconds:60},async(req,res)=>{
 ...
 try{
  ...
  if(route==='/events'&&req.method==='POST'){
   ...
   const raw=req.body?.event;if(!raw?.id||!/^[\w:.-]{1,160}$/.test(raw.id))throw failure(400,'일정 ID가 올바르지 않습니다.');
   ...
   const event=await db.runTransaction(async tx=>{
    ...
    let e;try{e=cleanEvent(raw,old,uid);}catch(err){throw failure(400,err.message);}
  ...
  if(route==='/subscriptions'&&req.method==='POST'){
   ...
   await collection.doc(key).set({subscription:{endpoint:sub.endpoint,keys:{auth:String(sub.keys.auth),p256dh:String(sub.keys.p256dh)}},updatedAt:Date.now()});res.json({ok:true});return;
  }
...
async function dispatch(ref,now){
 ...
 try{
  ...
  for(const uid of recipients){
   ...
   for(const sub of subs.docs){targets++;const key=uid+':'+sub.id;if(delivered.has(key))continue;
    ...
    try{await webpush.sendNotification(sub.data().subscription,JSON.stringify({title:'달님 · 일정 알림',body:e.title,eventId:e.id,deliveryId:ref.id}),{TTL:3600,urgency:'high',timeout:12000});delivered.add(key);await ref.update({delivered:FieldValue.arrayUnion(key)});}
    catch(err){if(err.statusCode===404||err.statusCode===410)await sub.ref.delete();else failures++;}
   }
  }
  ...
  await finish(failures?'failed':targets?'sent':'no-device');
