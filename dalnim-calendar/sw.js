...
self.addEventListener('push',e=>{let d={};try{d=e.data?.json()||{};}catch{}
 const url=new URL('./',self.registration.scope);if(d.eventId)url.searchParams.set('event',d.eventId);if(d.deliveryId)url.searchParams.set('delivery',d.deliveryId);if(d.investmentItemId)url.searchParams.set('investment',d.investmentItemId);
