...
function renderEditor(host, draft) {
  ...
  host.innerHTML = '<div class="extra-label">가족 일정 상세</div><div class="form-grid">'
    ...
    + field('목적지 / 장소', '<input data-detail="dest" maxlength="200" placeholder="여행지, 행사장 등" value="' + esc(draft.dest ?? '') + '">')
    + field('이동수단', '<input data-detail="transport" maxlength="100" placeholder="자동차 · 기차 · 비행기" value="' + esc(draft.transport ?? '') + '">')
    + field('숙소', '<input data-detail="lodging" maxlength="200" placeholder="숙소명 또는 예약 링크" value="' + esc(draft.lodging ?? '') + '">', true)
