...
function openEditor(id,patch={},anchor=null){
 ...
 // A phone-number-looking value offers a tap-to-call link; anything else offers Naver Map search — no field just for that.
 const renderLocationLink=()=>{
  const v=(form.location.value||'').trim(),link=$('#location-link');if(!link)return;
  if(!v){link.innerHTML='';return;}
  const digits=v.replace(/[^0-9+]/g,'');
  link.innerHTML=/^\+?\d{7,15}$/.test(digits)?'<a href="tel:'+esc(digits)+'">'+icon('link')+'전화 걸기</a>':'<a href="https://map.naver.com/p/search/'+encodeURIComponent(v)+'" target="_blank" rel="noopener">'+icon('link')+'네이버 지도에서 보기</a>';
 };
 form.location.addEventListener('input',renderLocationLink);renderLocationLink();
 $('#location-toggle')?.addEventListener('click',b=>{$('#location-block').style.display='block';b.target.remove();form.location.focus();});
 d.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>d.close());
