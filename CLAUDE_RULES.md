...
>
> **최우선 규칙: 모든 프로그램의 신규 개발·수정·확장 전에 반드시 `SYSTEM_PHILOSOPHY.md`, `CORE_ARCHITECTURE.md`, `DESIGN_SYSTEM.md`를 먼저 읽는다.**
> 개별 앱의 편의나 기능만 최적화하지 말고, 그 앱이 전체 투자 인식·추론 시스템에서 어떤 '감각'을 담당하며 종합추론 머리에 어떤 신호를 전달하는지 먼저 검토한다.
> AI를 포함한 어떤 하나의 프로그램·데이터원·API에도 전체 판단을 종속시키지 않는다. 하나가 없어도 기본 기능은 계속 작동해야 하며, 연결될수록 판단 품질이 높아지는 구조를 우선한다.
...
- 구조 기준 원본: `CORE_ARCHITECTURE.md`
- 디자인 기준 원본: `DESIGN_SYSTEM.md`
- 핵심 문장: **프로그램은 감각이고, 종합추론은 6번째 감각이다. 부분을 만들되 항상 전체를 본다.**
- 뉴스·유튜브·공시·매크로·가격/수급·기록보관실·AI 등은 각각 독립적인 감각/기능 모듈이다.
...
- AI가 없어도 공식지표·공시·가격·거래량·수급·금리·환율·규칙·통계로 기본 판단이 가능해야 한다.
- 새 기능은 반드시 "전체 시스템의 지능을 실제로 높이는가?"를 먼저 검토한다.
- 헌장과 세부 규칙이 충돌하면 `SYSTEM_PHILOSOPHY.md`를 우선한다. 구조 구현은 `CORE_ARCHITECTURE.md`, 디자인 구현은 `DESIGN_SYSTEM.md`를 따른다.

### 모든 작업 시작 전 확인
1. 이 프로그램은 어떤 감각인가?
2. 이번 수정으로 무엇을 더 빨리·정확하게 관찰하게 되는가?
3. 종합추론 머리에 어떤 표준 신호를 전달할 수 있는가?
4. 이 모듈이 없어도 전체 시스템은 계속 작동하는가?
5. 다른 모듈과 연결되면 어떤 새로운 추론이 가능한가?
6. AI 없이도 계산·규칙 기반 기본 기능은 어디까지 가능한가?
7. 화면을 열고 3초 안에 핵심을 알 수 있는가?
8. 현재 데이터 표현(숫자/표/차트/흐름도/타임라인)이 목적에 가장 적합한가?
9. 불필요한 버튼을 줄이거나 통합할 수 있는가?
10. PC·모바일·출력에서 같은 판단 구조가 유지되는가?
...
```
✅ 0. SYSTEM_PHILOSOPHY.md + CORE_ARCHITECTURE.md + DESIGN_SYSTEM.md 먼저 읽기
...
✅ 0-D. AI 없이도 기본 기능이 어디까지 가능한지 먼저 정의
✅ 1. 🐞 디버그 패널 + 버전 표시  ← 가장 먼저
✅ 2. 밝은 흰색/라이트 테마 (어두운 배경 절대 금지)
✅ 3. 카테고리 추가/삭제 항상 가능
✅ 4. AI 기능은 선택적 보조기능으로 설계
✅ 5. Firebase 표준 구조 사용
...
2. `CORE_ARCHITECTURE.md` 읽기
3. `DESIGN_SYSTEM.md` 읽기
4. 전체 시스템에서 앱의 감각 역할 확인
5. 내부/외부 재사용 후보 조사
6. 관련 파일 읽기
7. 기능+디자인 설계 검토
8. 구현